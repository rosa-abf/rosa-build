...
describe Projects::BuildListsController do
  ...
  shared_examples_for 'create build list' do
    ...
    it 'should be able to perform new action' do
      get :new, owner_with_name: "#{@project.owner.uname}/#{@project.name}"
    ...
    it 'should save correct commit_hash for branch based build' do
      post :create, {owner_with_name: "#{@project.owner.uname}/#{@project.name}"}.merge(@create_params).deep_merge(build_list: {project_version: "master"})
    ...
    it 'should save correct commit_hash for tag based build' do
      ...
      post :create, {owner_with_name: "#{@project.owner.uname}/#{@project.name}"}.merge(@create_params).deep_merge(build_list: {project_version: "4.7.5.3"})
    ...
    it 'should not be able to create with wrong project version' do
      lambda{ post :create, {owner_with_name: "#{@project.owner.uname}/#{@project.name}"}.merge(@create_params).deep_merge(build_list: {project_version: "wrong", commit_hash: nil})}.should change{@project.build_lists.count}.by(0)
    end

    it 'should not be able to create with wrong git hash' do
      lambda{ post :create, {owner_with_name: "#{@project.owner.uname}/#{@project.name}"}.merge(@create_params).deep_merge(build_list: {commit_hash: 'wrong'})}.should change{@project.build_lists.count}.by(0)
    end
  end
  ...
  context 'crud' do
    ...
    context 'for user' do
      before(:each) do
        ...
        @show_params = {owner_with_name: "#{@project.owner.uname}/#{@project.name}", id: @build_list.id}
