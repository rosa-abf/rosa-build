...
class Ability
  ...
  def initialize(user)
    ...
    can [:read, :all], BuildList, :project => {:visibility => 'open'}
    ...
    if user.guest? # Guest rights
      ...
      if user.user?
        ...
        can [:read, :owned, :all], BuildList, :user_id => user.id
        can [:read, :related, :all], BuildList, :project => {:owner_type => 'User', :owner_id => user.id}
        can [:read, :related, :all], BuildList, :project => {:owner_type => 'Group', :owner_id => user.group_ids}
        can([:read, :all], BuildList, read_relations_for('build_lists', 'projects')) {|build_list| can? :read, build_list.project}
